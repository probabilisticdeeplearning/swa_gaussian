...
class SyntheticGaussianData(torch.utils.data.Dataset):
    def __init__(self,
        ...
        self.resample_theta = resample_theta
        if self.file.exists() and reuse_data:
            ...
            if self.resample_theta:
                sampled_theta = np.random.multivariate_normal(
                    mean=self.theta_0, cov=self.cov_theta, size=n_samples)
                sampled_x = np.array([
                    np.random.multivariate_normal(mean=theta, cov=self.cov_x)
                    for theta in sampled_theta
                ])
            else:
                sampled_x = np.random.multivariate_normal(
                    mean=self.theta_0, cov=self.cov_x, size=n_samples)
...
def camvid_loaders(path,
    ...
    print(path)
    train_set = CamVid(root=path,
                       split='train',
                       joint_transform=joint_transform,
                       transform=transform_train,
                       **kwargs)
    ft_train_set = CamVid(root=path,
                          split='train',
                          joint_transform=ft_joint_transform,
                          transform=transform_train,
                          **kwargs)

    val_set = CamVid(root=path,
                     split='val',
                     joint_transform=None,
                     transform=transform_test,
                     **kwargs)
    test_set = CamVid(root=path,
                      split='test',
                      joint_transform=None,
                      transform=transform_test,
                      **kwargs)
    ...
    return {
        ...
        torch.utils.data.DataLoader(train_set,
                                    ...
                                    pin_memory=True),
        'fine_tune':
        torch.utils.data.DataLoader(ft_train_set,
...
def loaders(dataset,
    ...
    if dataset == 'SVHN':
        return svhn_loaders(path, batch_size, num_workers, transform_train,
                            transform_test, use_validation, val_size)
    else:
        ds = getattr(torchvision.datasets, dataset)
    ...
    if split_classes is not None:
        ...
        train_set.targets = np.where(
            train_set.targets[:, None] == c10_classes[split_classes]
            [None, :])[1].tolist()
        print('Train: %d/%d' %
