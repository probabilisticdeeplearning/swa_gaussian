...
class Test_LowRank_P_Diag(unittest.TestCase):
    def test_added_diag_lt(self, N=10000, p=20, use_cuda=False, seed=1):
        ...
        probe_vectors = torch.empty(lowrank_pdiag_lt.matrix_shape[-1],
                                    num_random_probes,
                                    dtype=lowrank_pdiag_lt.dtype,
                                    device=lowrank_pdiag_lt.device)
        ...
        print('diff between multiple linear_cg: ',
        ...
        log_det_term, = slq.evaluate(lowrank_pdiag_lt.matrix_shape,
